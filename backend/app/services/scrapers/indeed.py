        ...
        try:
            async with async_playwright() as p:
                ...
                for card in cards[:20]:
                    try:
                        titre_el = await card.query_selector("h2.jobTitle")
                        entreprise_el = await card.query_selector("[data-testid='company-name']")
                        ville_el = await card.query_selector("[data-testid='text-location']")
                        ...
                        link = f"https://fr.indeed.com{href}" if href and href.startswith("/") else href
